...
void GameWidget::gameReset() {
    ...
    m_enemies.clear(); m_pBullets.clear(); m_eBullets.clear(); m_pickups.clear(); m_pts.clear();
    m_wave = 0; m_score = 0; m_frame = 0; m_remain = 0;
    m_state = BREAK; m_breakTimer = m_breakLen;
}

void GameWidget::startWave() {
    int cnt = 6 + m_wave * 2; m_remain = cnt;   //  敌人数目，后续调整
    for (int i = 0; i < cnt; ++i) {
        EnemyUnit eu;
        float x = rf(80, width() - 80), y = rf(60, height() / 2 - 80);  //  敌人位置，后续调整
        int typ = (ri(0, 9) < 7) ? 0 : 1;   //  精英占比，后续调整
        eu.reset(x, y, typ, m_wave);
        eu.setBulletAdder([this](const Bullet& b) { m_eBullets.append(b); });
        m_enemies.append(eu);   //  添加一个对象到列表末尾
    }
}
...
void GameWidget::tick() {
        ...
        for (auto& e : m_enemies) {     //  碰撞检测
            ...
            if (dist(b.x, b.y, e.data().x, e.data().y) < b.radius + 18.0f) {
                ...
                if (e.data().hp <= 0) {
                    ...
                    m_remain--;
                    m_score += 150;   //    加分后续调整
                    genParticles(e.data().x, e.data().y, 20, Qt::red);  //  爆炸粒子数目后续调整
    ...
    for (auto& b : m_eBullets) {
        ...
        if (!m_player.isInv() && dist(b.x, b.y, m_player.x(), m_player.y()) < b.radius + m_player.radius()) {
            b.alive = false;
            m_player.hit();
            genParticles(m_player.x(), m_player.y(), 10, Qt::cyan);     //  10个青色粒子为受伤，后续调整
            if (m_player.hp() <= 0) { 
                m_state = OVER;
                return;
            }
    ...
    for (auto& pk : m_pickups) {
        ...
        if (dist(m_player.x(), m_player.y(), pk.x, pk.y) < 22) {        //  道具被拾取
            pk.alive = false;   
    ...
    m_pBullets.erase(   //  移除元素
        std::remove_if(m_pBullets.begin(), m_pBullets.end(),    //  移尾函数
            [](const Bullet& b) { return !b.alive; }),  //  lambda表达式
        m_pBullets.end());
...
void GameWidget::checkWave() {
    ...
    if (m_remain <= 0 && m_enemies.isEmpty()) {
        if (m_wave == m_totalWaves)  
            m_state = OVER; 
        else  m_state = BREAK; m_breakTimer = m_breakLen; 
    }
}

void GameWidget::timerEvent(QTimerEvent*) {
    if (m_state == MENU || m_state == OVER|| m_state == PAUSED) { update(); return; }
    if (m_state == BREAK) {
        m_breakTimer--;
        if (m_breakTimer <= 0) { m_wave++; startWave(); m_state = PLAY; }
        update(); return;
    }
    tick(); update();
}
...
void GameWidget::paintEvent(QPaintEvent*) {
    ...
    drawSkill(20, m_player.bombSkill(), "Bomb", 1);
    drawSkill(100, m_player.shieldSkill(), "Shield", 2);
    drawSkill(180, m_player.fireSkill(), "Fire+", 3);
