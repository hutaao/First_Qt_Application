...
void EnemyUnit::reset(float x, float y, int type, int wave) {
    ...
    m_data.type = type;
    m_data.baseX = x;
    m_data.moveAmp = (type == 0) ? 0.0f : 80.0f;
    m_data.shootInterval = qMax(20, 45 - wave * 4);
    m_data.shootTimer = 10 + rand() % m_data.shootInterval;
    m_data.alive = true;
    if (type == 0) {
...
void EnemyUnit::fireStraight(float x, float y, float vx, float vy, QColor color, float radius) {
    ...
    b.x = x; b.y = y;
    b.vx = vx; b.vy = vy;
    b.radius = radius; b.color = color; b.damage = 1;
    b.type = 1; b.alive = true;
    m_addBullet(b);
}
