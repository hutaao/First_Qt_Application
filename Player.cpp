...
void Player::reset(float startX, float startY) {
    ...
    m_invTimer = 0;
    m_dmg = 1;
    m_bomb.cd = m_bomb.active = 0;
    m_shield.cd = m_shield.active = 0;
    m_fire.cd = m_fire.active = 0;
}

void Player::setBulletAdder(std::function<void(const Bullet&)> adder) {
    m_addBullet = adder;
}
...
void Player::tryShoot() {
    ...
    Bullet b;
    b.color = Qt::yellow; b.radius = 3; b.damage = m_dmg; b.alive = true; b.type = 0;
    b.vx = 0; b.vy = -8;
    b.y = m_y - 20;
...
void Player::useSkill(int id) {
    if (id == 1 && m_bomb.cd <= 0 && m_bomb.active == 0) {
...
void Player::updateSkills() {
    ...
    if (m_shield.active > 0) { m_shield.active--; m_invTimer = 3; }
...
void Player::getPickup(int type) {
    switch (type) {
    case 0: m_hp = qMin(m_maxHp, m_hp + 3); break;
    case 1: m_invTimer = qMax(m_invTimer, 120); break;
    case 2: m_fire.active = 300; m_dmg = 2; break;
    }
}
